...
const CommentForm: React.FC<CommentFormProps> = ({ handleClose, open, fetchComment, initialData, commentId }) => {
    ...
    const {
        ...
        defaultValues: initialData || { content: "", rating: 3.5 },
    });
    ...
    const onSubmit: SubmitHandler<IFormInput> = async (data) => {
        try {
            ...
            let res;
            if (commentId) {
                res = await axios.put(`http://localhost:3000/comment/${commentId}`, {
                    content,
                    rating,
                });
            } else {
                res = await axios.post("http://localhost:3000/comment", {
                    userId: user?._id,
                    productId: id,
                    content,
                    rating,
            ...
            if (res.status === 200) {
                ...
                reset();
                fetchComment();
                toast.success(`${commentId ? "Cập nhật" : "Thêm"} bình luận thành công`, {
                    position: "top-right",
                    autoClose: 2000,
                });
            }
        ...
        <BootstrapDialog onClose={handleClose} aria-labelledby="customized-dialog-title" open={open}>
            ...
            <DialogContent dividers>
                <Typography variant="h6" sx={{ marginBottom: 3, fontSize: "1.8rem" }}>
                    {commentId ? "Cập nhật" : "Viết"} đánh giá mới ✏️
                </Typography>
                <form onSubmit={handleSubmit(onSubmit)}>
                    ...
                    <Box>
                        ...
                        <Controller
                            name="rating"
                            control={control}
                            defaultValue={initialData ? initialData.rating : 3.5}
                            render={({ field }) => (
                                <Rating
                                    {...field}
                    ...
                    <Box sx={{ display: "flex", justifyContent: "space-between", pt: 2 }}>
                        ...
                        <Button type="submit" variant="contained">
                            {commentId ? "Cập nhật" : "Gửi"} đánh giá
                        </Button>
                    </Box>
                </form>
            </DialogContent>
